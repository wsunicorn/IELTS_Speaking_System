...
- **State machine dùng XState** thay vì tự viết reducer — tránh race condition giữa các nguồn async (STT worker / Claude stream / TTS playback).
...
| State machine | **XState** | `idle/listening/thinking/speaking` có nhiều nguồn async (STT worker, Claude stream, TTS) dễ race condition nếu tự viết reducer — XState đảm bảo transition hợp lệ bằng cấu trúc |
...
Nếu chờ Claude trả lời **xong toàn bộ câu** rồi mới bắt đầu TTS, mỗi lượt hội thoại có thể "im lặng chết" 2–4 giây — phá cảm giác đang nói chuyện với người thật.

**Cách xử lý:**
1. Gọi Claude API ở chế độ **streaming** (qua proxy).
2. Tách text stream thành từng câu hoàn chỉnh (theo dấu câu `. ! ?`).
3. Ngay khi có câu đầu tiên → đưa vào TTS (Kokoro/HeadTTS) synthesize + phát **ngay**, trong lúc Claude vẫn đang generate các câu tiếp theo.
...
- **Phase 4 — Bộ não:** Cloudflare Worker proxy giữ key → vòng lặp hội thoại examiner (Part 1). State machine **XState** `idle/listening/thinking/speaking`. Claude API **streaming** + **progressive TTS** (tách câu, phát ngay câu đầu — mục 4.7) ngay từ phase này, không để dồn lại xử lý sau. Barge-in **không** làm ở phase này (ngoài scope MVP).
...
- **Stack miễn phí, in-browser, đã chứng minh chạy chung:** TalkingHead + Ready Player Me (avatar khẩu hình) · HeadTTS/Kokoro (giọng) · Moonshine+Whisper (nghe) · Claude API (bộ não) · Three.js audio-reactive (hiệu ứng) · XState (state machine).
